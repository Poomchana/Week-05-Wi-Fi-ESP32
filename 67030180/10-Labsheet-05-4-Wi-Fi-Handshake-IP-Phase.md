...
| :---: | :--- | :---: | :---: | :---: | :--- |
| **5.4.1** | Password ถูกต้อง | เกิด | เกิด | 	TEST PASSED | |
| **5.4.2** | Password ผิด | เกิด | ไม่เกิด | 	TEST FAILED | 15 (0x0F)(WIFI_REASON_4WAY_HANDSHAKE_TIMEOUT) |
...
- ช้เทคนิค Challenge-Response โดยทั้งสองฝั่งนำรหัสผ่านที่มี มาผสมกับค่าสุ่ม (ANonce, SNonce) เพื่อคำนวณค่าตรวจสอบ (MIC) แล้วส่งเฉพาะค่าสุ่มกับ MIC ไปเช็คกัน หากตรงกันแสดงว่าถือรหัสผ่านเดียวกันโดยไม่ต้องส่ง Passphrase จริง
...
- PMK (คีย์แม่บท): ถอดมาจากการนำ Passphrase + SSID มาแฮช (ค่าคงที่)
PTK (คีย์ชั่วคราว): นำ PMK มาผสมกับค่าสุ่มและ MAC Address เพื่อสร้างขึ้นใหม่ทุกครั้งที่เชื่อมต่อ
ความสัมพันธ์: PMK เป็นตัวสร้าง PTK โดย PTK จะถูกนำไปใช้เข้ารหัส/ถอดรหัสเฟรมข้อมูลจริง
...
- CONNECTED เกิดขึ้นตั้งแต่จบ Phase 3 (Association) ซึ่งเป็นการต่อระดับวิทยุ/ลิงก์ โดยยัง ไม่ได้เช็ครหัสผ่าน
พอเริ่มทำ Phase 4 (4-Way Handshake) เพื่อเช็ครหัสผ่าน แล้วพบว่าผิด จึงค่อยตัดการเชื่อมต่อและส่ง DISCONNECTED ตามมา
...
- จะหยุดที่: Phase 4 (เชื่อมต่อและเข้ารหัสสำเร็จแล้ว)
จะไม่เกิด Event: IP_EVENT_STA_GOT_IP (เพราะไม่ได้รับหมายเลข IP)
