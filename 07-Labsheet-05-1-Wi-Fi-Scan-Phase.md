...
| :---: | :--- | :---: | :---: | :---: |
| **5.1.1** | สแกนทั่วไปทุก Channel | Success (ESP_OK) | 12 | ไม่ได้ระบุใน Log (All Channels Scan) |
| **5.1.2** | กำหนดสแกนเฉพาะ Channel 1 | Success (ESP_OK) | 5 | 200 |
| **5.1.3** | กำหนดสแกน SSID ที่มีจริง | Success (ESP_OK) | 4 | 2499 |
...
| :---: | :--- | :--- | :---: | :---: | :--- |
| 1 | KMITL-WIFI | 78:17:BE:C0:7D:A1 | -47 dBm | 1 | OPEN (No Password) |
| 2 | KMITL-Legacy | 78:17:BE:C0:7D:A0 | -49 dBm | 1 | WPA2_ENTERPRISE |
| 3 | KMITL-IoT | 78:17:BE:C0:7D:A2 | -49 dBm | 1 | WPA2_PSK |
| 4 | KMITL-Legacy | 78:17:BE:C0:66:60 | -67 dBm | 11 | WPA2_ENTERPRISE |
| 5 | KMITL-IoT | 78:17:BE:C0:66:62 | -67 dBm | 11 | WPA2_PSK |
...
- ลดเวลาอย่างไร: สแกนแค่ช่องเดียว ไม่ต้องไล่หาครบทุกช่อง (1–13) ทำให้เสร็จไวมาก (ใช้เพียง 200 ms)
ข้อจำกัด: ไม่เห็น Wi-Fi ที่ปล่อยอยู่ในช่องความถี่อื่น
...
- esp_wifi_scan_start(): คืนค่า ESP_OK (สแกนสำเร็จ)
esp_wifi_scan_get_ap_num(): คืนค่า ESP_OK และได้จำนวน AP = 0
esp_wifi_scan_get_ap_records(): คืนค่า ESP_OK แต่ไม่มีข้อมูลบันทึกออกมา
...
- -45 dBm แรงและเสถียรกว่า (ยิ่งติดลบน้อย ยิ่งสัญญาณแรง)
4. เหตุใดการดึงค่า `authmode` (`wifi_auth_mode_t`) จากโครงสร้าง `wifi_ap_record_t` จึงมีความสำคัญต่อการเตรียมการในเฟสถัดไป (Authentication & Association
Phase)?
- เพื่อให้รู้ว่า AP ใช้ระบบความปลอดภัยแบบไหน จะได้เลือกวิธีใส่รหัสผ่านและต่อ Wi-Fi ได้ถูกต้อง
